        ...
        if len(fp) > len("c:") and len(self.text_box5.text()) > 1 and self.text_box7.text().isdigit(): #this determines whether a complete file path has been submitted.
            ...
            while True: #While loop used to update the tree image.
                if os.path.getsize(self.distance_matrix_file) == 0: #If the distance matrix file is empty.
                    ...
                    QtTest.QTest.qWait(60000) #Wait every X amount of miliseconds after each iteration.
                    if progress > 99.9:
                        msg2.setText("The analysis has been completed. The image has been provided."
                                     "The program shuts down automatically")
                        msg2.setIcon(QMessageBox.Information)
                        msg2.setWindowIcon(self.logo_path)
                        y = msg2.exec_()  # Shows message box
