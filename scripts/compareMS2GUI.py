...
class functional (Gui):
    ...
    def submitmethod(self):
        ...
        msg2 = QMessageBox()
        msg2.setWindowTitle('Analysis status')
        msg3 = QMessageBox()
        ...
        if len(fp) > len("c:") and len(self.text_box5.text()) > 1 :#and self.text_box7.text().isdigit(): #this determines whether a complete file path has been submitted.
            ...
            while True: #While loop used to update the tree image.
                if os.path.getsize(self.distance_matrix_file) == 0: #If the distance matrix file is empty.
                    ...
                    QtTest.QTest.qWait(1000) #checks the file every second.
                else:
                    progress = self.tree_processing(self.distance_matrix_file) #Creates new image each iteration of loop.
                    QtTest.QTest.qWait(10000) #Wait every X amount of miliseconds after each iteration.
                    if progress > 99.9: #If the analysis is completed.
                        msg3.setText("The analysis has been completed. The tree image has been provided.\n")
                        msg3.setIcon(QMessageBox.Information)
                        z = msg3.exec_()  # Shows message box
    ...
    def tree_processing(self,distance_matrix_file):
        ...
        '''
        self.tree= tree_analysis(distance_matrix_file)#,int(self.text_box7.text())) #Calls tree screen and passes the distance matrix file.
                                                                                  # Alongside total amount of samples.
        percentual_progress = (self.tree.upgma_result.label_count / 8 ) * 100 #/ int(self.text_box7.text())) * 100  # Divide with amount of samples present in input file. 100 is generic number used to test.
